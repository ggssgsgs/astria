...
export default {
  ...
  methods: {
    buildSketch(p) {
      ...
      this.myp5 = new P5(function (p5) {
        ...
        //spiral
        function changing() {
          angleForSize = p5.frameCount * 0.02;
        }
        ...
        p5.draw = (_) => {
          ...
          if (!trace) {
            ...
            setGradient(0, 0, p5.width, p5.height, c1, c2, 1);
            p5.push();
  ...
  mounted() {
    ...
    //     myCanvas.position(0, 0);
    //     myCanvas.style("z-index", 2);
    //     p5.imgLOGO = p5.loadImage("img/LOGO.png");
    //     p5.imgMoonBg = p5.loadImage("img/moonBg");
    //     p5.imgStarHolder = p5.loadImage("img/starHoolder.png");
    //     rad = p5.height * 0.2; // compute radius for central circle
    //     p5.background(255); // clear the screen
    //     for (let i = 0; i < sines.length; i++) {
    //       sines[i] = p5.PI; // start EVERYBODY facing NORTH
    //     }
    ...
    // // const P5 = require("p5");
    // this.myp5 = new P5(script);

    this.buildSketch(this.$props);
  },
};
</script>
